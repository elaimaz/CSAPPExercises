...
word f_predPC = [
	...
	f_icode == IJXX && f_valC < f_valP : f_valC;	# Backward branch (taken)
	f_icode == IJXX && f_valC >= f_valP : f_valP;	# Forward branch (not-taken)
	f_icode == ICALL : f_valC;	# conditional Jump
	1 : f_valP;
];
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && ((E_valC < E_valP && !e_Cnd) || (E_valC >= E_valP && e_Cnd))) ||
